/**
 * Script para importar CSV usando SOLO las columnas que existen en la tabla Lead
 */
...
async function importarCSVSoloColumnasExistentes() {
  try {
    console.log('📥 IMPORTANDO CSV CON FORMATO CORRECTO Y SOLO COLUMNAS EXISTENTES...\n');
    ...
    console.log('📥 Paso 3: Importando con nombres formateados correctamente...');
    ...
    for (let i = 0; i < datos.length; i++) {
      try {
        ...
        // Formatear datos SOLO si existen en el CSV
        ...
        // Crear el lead CON SOLO LAS COLUMNAS QUE EXISTEN EN LA TABLA
        const leadData = {
          nombre: nombreFormateado,
          telefono: telefonoFormateado, // null si no existe
          email: null, // NO generar email si no está en el CSV
          estado,
          origen: 'csv',
          ingresos,
          zona: zonaOriginal || null, // null si no existe
          dni: dni || null,
          notas: notasTexto || null
          // NO incluir 'trabajo' porque no existe en la tabla
        };
        ...
        if (errores > 10) {
          console.log('   ⚠️  Demasiados errores, verificando estructura...');
          break;
        }
      }
    }
    ...
    echeverriaLeads.forEach(lead => {
      console.log(`   ✅ "${lead.nombre}" (Teléfono: ${lead.telefono || 'N/A'}, Email: ${lead.email || 'N/A'})`);
    });
    ...
    // Verificar que NO se agregaron emails inventados
    const leadsConEmail = leadsFinales.filter(lead => lead.email);
    console.log(`\n📧 Verificación de emails:`);
    console.log(`   Leads con email: ${leadsConEmail.length} (debería ser 0 si no había emails en el CSV)`);
    
    // Verificar que NO se agregaron teléfonos inventados
    const leadsConTelefono = leadsFinales.filter(lead => lead.telefono);
    console.log(`\n📞 Verificación de teléfonos:`);
    console.log(`   Leads con teléfono: ${leadsConTelefono.length} (solo los que tenían teléfono en el CSV)`);
    ...
    console.log(`\n✅ BASE DE DATOS CORRECTA Y FORMATEADA!`);
    ...
    console.log(`🚫 SIN datos inventados (emails, teléfonos, etc.)`);
    console.log(`📋 Solo usa columnas que existen en la tabla Lead`);
...
importarCSVSoloColumnasExistentes();
